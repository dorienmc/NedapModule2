...
public class SlidingWindowDataTransferProtocol extends IRDTProtocol {
	...
	static final int REC_TIMEOUT = TIMEOUT/4;
	private int LastFrameReceived = -1;
	private int ReceiveWindowSize = 15;
	private Map<Integer, Integer[]> bufferedPackets;
	//private int LargestAcceptableFrame = LastFrameReceived + ReceiveWindowSize;
	...
	public void sender() {
		...
		while(filePointer < fileContents.length) {
			...
			assert(isInSlidingWindow(LastFrameSent));
			Integer[] pkt = createPacket(filePointer, fileContents, LastFrameSent);
   ...
   * Wait for acknowledgment of given packet, if received stop the timeout of that
	...
	public void sendEOF() {
		...
		while (!stop) {
			...
			if(receivedPkt != null && receivedPkt[0] == ACK_HEADER && receivedPkt[1] == ENDOFFILE_HEADER) {
	...
	public void receiver() {
		...
		timeOut.setTime(timeOut.getTime() + REC_TIMEOUT);
		while (!stop) {
